...
class AnchorsGenerator(nn.Module):
  ...
  '''
    计算/读取所有生成的anchors对应原图的anchors的坐标信息
  '''
  ...
  def forward(self, image_list, feature_maps):
    ...
    # torch.size([25,38]) ,预测图像的高和宽
    grid_sizes = list([feature_map.shape[-2:] for feature_map in feature_maps])

    '''
       # 获取输入图像的height和width，transform后输入backbone的图像大小
       # torch.size([800,1216])
    '''
    ...
    '''根据提供的sizes和aspect_ratios生成anchors模板'''
    # [sizes * aspect_ratios , 4]
    self.set_cell_anchors(dtype, device)

    '''
      计算/读取所有anchors的坐标信息（这里的anchors信息是映射到原图上的所有anchors信息，不是anchors模板）
      得到的是一个list列表，对应每张预测特征图映射回原图的anchors坐标信息
      grid_size是features_map的大小宽高
      stride是传入的和transform后的图像的大小比例
    '''
    # 得到所有特征图上映射到原图的anchors
...
class RegionProposalNetwork(torch.nn.Module):
  ...
  def assign_targets_to_anchors(self, anchors, targets):
    ...
    for anchors_per_image, targets_per_image in zip(anchors, targets):
      ...
      if gt_boxes.numel() == 0:
        ...
        '''
        match_shape = matched_idxs.clamp(min=0)
        # 取出所有负样本，保留正样本和背景
        matched_gt_boxes_per_image = gt_boxes[matched_idxs.clamp(min=0)]

        '''
          有一个疑问？
          matched_idxs 为一张图片的anchors和gtbox做的iou值，
          labels_per_image是与matched_idxs一样的张量
        '''
  ...
  def _get_top_n_idx(self, objectness, num_anchors_per_level):
    ...
    # 遍历每个预测特征层上的预测目标概率信息
    '''
     objectness.split(num_anchors_per_level, 1):将预测值在所有预测特征分割
    '''
    for ob in objectness.split(num_anchors_per_level, 1):
      ...
      # 选取Tensor中最大的前pre_nms_top_n索引和值
  ...
  def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
    ...
    # 所有的预测概率  [batch,image_per_anchor]
    ...
    # 在所有的索引值下获取对应的前pre_nms_top_n的索引值
